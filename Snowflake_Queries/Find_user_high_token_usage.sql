...
----- High Token usage --------------------
with prompts (user_id,prompt,tokens) as (
select * from values 
(1,'Write a blog outlint',120),
(1,'Generate SQL Query',80),
(1,'Summarize an article',200),
(2,'Cretae Resume Bullet',60),
(2,'Improve Linked I n Bio' , 70),
(3,'Explain neural network',300),
(3,'Generate Interview Q & A',250),
(3,'Write Cover Letter',180),
(3,'Optimize Pyhton Code ' , 220)
t(user_id,prompt,tokens)
),
average (count,average,user_id) as (
    select count(*) as count , ROUND( avg(tokens),2) as average,user_id from prompts
    group by user_id having count >2 
) 

select distinct prompts.user_id as user_id,
 average.count as count , 
 average.average as avg_tokens
from prompts inner  join average
on prompts.user_id = average.user_id 
where  prompts.tokens > average.average order by  avg_tokens desc, user_id asc;
