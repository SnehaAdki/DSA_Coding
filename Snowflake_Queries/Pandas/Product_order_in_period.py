...
def list_products(products: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
    values = orders[orders['order_date'].between(pd.to_datetime('2020-02-01') , pd.to_datetime('2020-02-29') , inclusive='both')].groupby(by=['product_id'] , as_index=False).agg(
        r_unit = ('unit' , 'sum')
    ).rename(columns={'r_unit':'unit'})
    result = values[values['unit']>=100]
    final_result = result.merge(products , on='product_id' , how='left' , suffixes=('_left', '_right'))[['product_name','unit']]
    return final_result
...
print(list_products(Products , Order))