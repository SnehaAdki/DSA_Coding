...
# +---------+---------------+------------+
# | user_id | prompt_count  | avg_tokenss |
# +---------+---------------+------------+
# | 3       | 4             | 237.5      |
# | 1       | 3             | 133.33     |
# +---------+---------------+------------+
...
def find_users_with_high_tokens(prompts: pd.DataFrame) -> pd.DataFrame:
    prompt = prompts.groupby(by = ["user_id"]).agg(
        prompt_count = ('prompt' , 'count'),
        avg_tokens = ('tokens' , 'mean')
    ).reset_index()
    
    prompt['avg_tokens'] = prompt['avg_tokens'].round(2)
    prompt = prompt[prompt['prompt_count'] >= 3 ]

    merged_values= prompts.merge(prompt , on='user_id',how='inner')
    final_df = merged_values[merged_values['tokens'] > merged_values['avg_tokens']]
    final_df = final_df.drop_duplicates(subset=['user_id'])[['user_id' , 'prompt_count' , 'avg_tokens']]
    final_df = final_df.sort_values(['avg_tokens', 'user_id'], ascending=[False, True])
    return final_df


prompt = pd.DataFrame({
    "user_id" : [1,1,1,2,2,3,3,3,3],
    "prompt" : ["Write a blog outline" , "Generate SQL query" , "Summarize an article " , "Create resume bullet" , "Improve LinkedIn bio" , 
...
print(find_users_with_high_tokens(prompt))